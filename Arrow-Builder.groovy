    ...
    sh  '''#!/bin/bash
            ...
            if [ -f $TO_UPLOAD ]; then
                if [ '''+env.test_build+''' = "yes" ]; then
                    if [ '''+IS_COMMUNITY+''' = "true" ]; then
                        script -q -c "rsync -rav --info=progress2 $TO_UPLOAD root@10.0.0.200:/mnt/HDD1/builds/arrow-$(echo '''+env.TG_ARROW_VERSION+''' | cut -d "v" -f 2)/community_experiments/'''+DEVICE+'''/" | stdbuf -oL tr '\r' '\n'
                    else
                        script -q -c "rsync -rav --info=progress2 $TO_UPLOAD root@10.0.0.200:/mnt/HDD1/builds/'''+VERSION+'''/experiments/'''+DEVICE+'''/" | stdbuf -oL tr '\r' '\n'
                    fi
                    if [ $? -eq 0 ]; then
                    ...
                    if [ '''+IS_COMMUNITY+''' = "true" ]; then
                        script -q -c "rsync -rav --info=progress2 $TO_UPLOAD root@10.0.0.200:/mnt/HDD1/builds/arrow-$(echo '''+env.TG_ARROW_VERSION+''' | cut -d "v" -f 2)/community/'''+DEVICE+'''/" | stdbuf -oL tr '\r' '\n'
                    else
                        script -q -c "rsync -rav --info=progress2 $TO_UPLOAD root@10.0.0.200:/mnt/HDD1/builds/'''+VERSION+'''/official/'''+DEVICE+'''/" | stdbuf -oL tr '\r' '\n'
                    fi
                    if [ $? -eq 0 ]; then
